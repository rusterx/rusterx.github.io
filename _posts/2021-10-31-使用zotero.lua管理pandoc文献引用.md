...
使用`--citeproc`这种方式需要首先准备好`bib`文献。如果只编写docx文档。那么还可以使用`Better BibTeX`的作者编写的`zotero.lua`过滤器。只要在markdown文件中插入`[@ref_key]`。然后使用下边的代码转换：
...
可见整个过程不涉及bib文件。对于其中涉及图片、表格以及公式的引用，仍然使用之前文章讲到的`pandoc-fignos`、`pandoc-tablenos`以及`pandoc-eqnos`过滤器即可。
